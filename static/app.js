...
function renderDetail(p, tab, runtime, deps, isNew) {
  ...
  const info = `${runActions}<div class="detail-overview"><div><small>项目名称</small><b>${esc(p.name || "未命名项目")}</b></div><div><small>运行状态</small>${status(p)}</div><div><small>运行环境</small><b>${esc((p.runtime || "python").toUpperCase())}</b></div>${gitInfo}<div><small>项目目录</small><b class="mono">${esc(p.root_path || "-")}</b></div><div><small>运行进程</small><b>PID ${x.pid || "-"} · CPU ${x.cpu || 0}% · ${bytes(x.memory)}</b></div><div><small>监听端口</small><b>${x.ports?.join(", ") || "-"}</b></div><div class="service-addresses"><small>服务地址</small>${accessLinks(x.ports || [])}</div></div>${p.is_git ? `<h3>代码更新历史</h3>${history}` : ""}`;
...
function setupProjectOptions(project) {
  ...
  if (rootLabel && !$("#runtimeConfig")) {
    ...
    runtime.innerHTML = `<h3>运行环境</h3><div class="form-grid"><label>运行环境<select id="runtime"><option value="python" selected>Python</option></select></label><label>Python 命令<input id="python_executable" value="${esc(project.python_executable || "python3")}" placeholder="python3"></label><label>虚拟环境目录<input id="venv_path" value="${esc(project.venv_path || ".venv")}" placeholder=".venv"><span class="hint">相对项目目录</span></label><label>pip 依赖文件<input id="requirements_file" value="${esc(project.requirements_file || "requirements.txt")}" placeholder="requirements.txt"><span class="hint">相对项目目录</span></label></div><label class="check"><input id="auto_install_dependencies" type="checkbox" ${project.auto_install_dependencies ? "checked" : ""}> 更新代码时自动安装 pip 依赖</label>`;
...
async function saveProject(e) {
  e.preventDefault();
  const data = {};
  ["id", "name", "root_path", "branch", "start_command", "stop_command", "pre_deploy_hook", "post_deploy_hook", "env_vars", "git_webhook_secret", "runtime", "python_executable", "venv_path", "requirements_file"].forEach(
    (k) => (data[k] = $("#" + k).value),
  );
